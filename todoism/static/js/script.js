$(document).ready(function () {
    ...
    function new_item(e) {
        var $input = $('#item-input'); // assuming this is the input for title
        var $descriptionInput = $('#item-notes'); // assuming input for description
        var $categoryInput = $('#item-category'); // assuming input for category
        var $importanceInput = $('#item-importance');
        var title = $input.val().trim();
        var description = $descriptionInput.val().trim();
        var category = $categoryInput.val().trim();
        var importance = $importanceInput.val().trim()
        
        // Check if Enter key is pressed and that the title is not empty
        ...
        // Create an object with all properties of the item
        var itemData = {
            body: title,
            notes: description,
            category: category,
        ...
        $.ajax({
            type: 'POST',
            url: new_item_url,
            data: JSON.stringify(itemData), // Send the item data as JSON
            contentType: 'application/json;charset=UTF-8',
            success: function (data) {
                M.toast({html: data.message, classes: 'rounded'}); // Show a success message
                $('.items').append(data.html); // Append the new item to the list
                activeM(); // Reinitialize Materialize components
                refresh_count(); // Refresh the count of items
            }
        });
    }
